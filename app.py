...
def _osm_embed_html(lat: str, lon: str, height: int = 450, zoom: int = 17) -> str:
    lat_f = float(lat)
    lon_f = float(lon)
    delta = 0.003
    bbox = f"{lon_f - delta},{lat_f - delta},{lon_f + delta},{lat_f + delta}"
    return f"""
    <iframe width="100%" height="{height}" frameborder="0" scrolling="no"
...
def _download_static_map(lat: str, lon: str, dest_path: str,
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "TSSR-Automator/1.0"})
...
def image_dialog(slot: str, label: str, extracted: list[str]):
    ...
    method = st.radio(
        ...
        key=f"d_method_{slot}", horizontal=True, label_visibility="collapsed",
    )
...
site_id = st.selectbox("PLAID", options=[""] + plaids, index=current_idx,
...
if st.session_state.site_data:
    ...
    st.caption("Copy the prompt below, feed it to **Gemini** or **ChatGPT** "
    ...
    with st.expander("📋 Step 1 — Copy this prompt", expanded=False):
        st.code(ai_helper.build_prompt(), language="text")

    with st.expander("📥 Step 2 — Paste AI response",
                     expanded=not st.session_state.get("ai_applied", False)):
        ai_response = st.text_area("Paste the AI's JSON response here:", height=200,
                                   key="ai_json_input",
            ...
            if st.button("✔ Apply AI fields", key="apply_ai",
                ...
                if not ai_response.strip():
                    ...
                    if not parsed:
                        ...
                        merged = core.merge_ai_fields(st.session_state.site_data, clean)
                        st.session_state.site_data = merged
                        st.session_state["ai_applied"] = True
                        persist()
                        if warnings:
                            st.warning(f"⚠️ Applied with {len(warnings)} warnings:\n\n"
    ...
    with st.expander("🔍 Preview merged site data", expanded=False):
        site = st.session_state.site_data
        permits = get_permits_for_towerco(site.get("towerco", ""))
        display_fields = [
...
if st.session_state.site_data:
    ...
    try:
        ...
        st.caption(f"🔧 `load_calc_page` loaded from: "
        ...
        if not hasattr(load_calc_page, "render"):
            st.error("❌ `load_calc_page` has no `render` function. "
        ...
        st.warning(f"⚠️ Load calculator module not available: {e}\n\n"
...
if st.session_state.site_data:
    ...
    st.caption("Explore the site location using its coordinates. "
    ...
    if not lat or not lon:
        ...
        tab_map, tab_street, tab_help = st.tabs([
            "🗺️ Open Street Map", "📸 Google Street View", "💡 How to use",
        ])

        with tab_map:
            st.markdown("**Vicinity map preview**")
            st.caption("Zoom in/out and pan. When you're happy with the view, "
                       "click 'Use as Vicinity Map' below to save a static snapshot.")
            components.html(_osm_embed_html(lat, lon, height=450), height=470)

            col_a, col_b = st.columns([1, 1])
            with col_a:
                if st.button("🖼 Use as Vicinity Map", key="use_osm_as_vicinity",
                    ...
                    if result:
                        ...
                        st.error("❌ Static map service unavailable. "
                                 "Use the 'How to use' tab for manual capture.")
            with col_b:
                st.markdown(f"[🔗 Open larger map ↗]"
                            f"(https://www.openstreetmap.org/?mlat={lat}&mlon={lon}"
                            f"#map=17/{lat}/{lon})")

        with tab_street:
            st.markdown("**Street View preview**")
            st.caption("Street View can't be embedded without a Google API key. "
                       "Click the button below to open it in a new tab, then "
                       "screenshot and paste it back into the app.")
            gmaps_embed_url = (f"https://maps.google.com/maps?q={lat},{lon}"
                               f"&t=k&z=17&output=embed")
            components.html(
                f"""<iframe width="100%" height="450" frameborder="0"
                    style="border: 1px solid #ccc; border-radius: 8px;"
                    src="{gmaps_embed_url}" allowfullscreen></iframe>""",
                height=470,
            )
            st.markdown("### Open in Google Maps")
            c1, c2 = st.columns(2)
            with c1:
                st.markdown(f"[🗺️ Satellite view ↗]"
                            f"(https://www.google.com/maps/@{lat},{lon},17z/data=!3m1!1e3)")
            with c2:
                st.markdown(f"[📸 Street View ↗]"
        ...
        with tab_help:
            ...
            **Option 1 — Auto-capture vicinity map (fastest)**
            ...
            3. The app downloads a static map PNG and saves it to the vicinity map slot.
            ...
            **Option 2 — Manual screenshot (for any map)**
            ...
            3. Take a screenshot (`Win + Shift + S`, `Cmd + Shift + 4`, etc.)
            4. Come back to this app → **Section 5 · Images**
            5. Click any slot → Choose **📋 Paste (Ctrl+V)** → paste

            **Option 3 — Satellite view**
            Use the **🗺️ Satellite view** link in the Street View tab.
        ...
        with qc1:
            if st.button("🖼 Capture as Vicinity Map", key="quick_vicinity",
                         use_container_width=True):
                workdir = ensure_workdir()
        ...
        with qc2:
            if st.button("📸 Capture as Site Photo 1", key="quick_site_photo",
                         use_container_width=True):
                workdir = ensure_workdir()
        ...
        with qc3:
            if st.button("🌐 Open in new tab", key="open_new_tab",
                         use_container_width=True):
                st.markdown(f"**→ [Click to open Google Maps]"
                            f"(https://www.google.com/maps/@{lat},{lon},17z)**",
                            unsafe_allow_html=True)
                st.caption("Right-click → **Open in new tab**, then screenshot.")
...
if st.session_state.site_data:
    ...
    if st.session_state.ericsson_docx is None:
        uploaded_pdf = st.file_uploader("Upload Ericsson TSSR PDF",
                                        type=["pdf"], key="ericsson_pdf_upload")
        if uploaded_pdf:
            workdir = ensure_workdir()
        ...
        st.success(f"✅ Ericsson TSSR loaded "
...
if st.session_state.site_data:
    ...
    filled = sum(1 for slot, _ in IMAGE_SLOTS
                 if st.session_state.image_map.get(slot)
                 and Path(st.session_state.image_map[slot]).exists())
    st.caption(f"**{filled} / {len(IMAGE_SLOTS)}** slots filled. "
    ...
    for r in range(rows):
        ...
        for c in range(COLS):
            ...
            with cols[c]:
                ...
                if st.button(f"{icon}  {label}", key=f"open_{slot}",
                             use_container_width=True,
    ...
    for key, label, default_unit, default_qty in MATERIALS:
        ...
        with c1:
            unit_val = st.text_input(
                ...
                key=f"input_{key}_unit", label_visibility="collapsed")
        with c2:
            qty_val = st.text_input(
                "qty",
                value=st.session_state.materials.get(f"{key}_qty", default_qty),
                key=f"input_{key}_qty", label_visibility="collapsed")
...
if st.session_state.site_data:
    ...
    if st.button("⚙️ Generate DOCX", type="primary", use_container_width=True):
        ...
        all_assets = {**st.session_state.image_map, **st.session_state.materials}
        ...
        with st.spinner("Rendering template…"):
            ...
            out_name = core.make_output_name(
                ctx.get("site_id", "SITE"), ctx.get("site_name", "TSSR"))
