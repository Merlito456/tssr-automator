...
def fill_template(
    ...
    def put(label: str, value, *, row_offset: int = 0, col_offset: int = 8):
        """Write `value` at the cell `col_offset` columns to the right of the label."""
        cell = label_to_cell.get(label)
        if not cell:
            return
        r, c = cell.row, cell.column
        ws.cell(row=r + row_offset, column=c + col_offset).value = value

    # C7.1 block (label -> value is 8 cols right in the sample layout)
    put("1) EXISTING RECTIFIER SYSTEM BRAND - MODEL",         data["rectifier_brand"])
    put("2) MAXIMUM RECTIFIER MODULES / INSTALLED",           data["max_modules"])
    put("3) RECTIFIER MODULE RATING, WATTS / AMPS",           data["module_rating_w"])
    put("4) BATTERY BRAND / VOLTAGE RATING",                  f'{data["battery_brand"]} {data["battery_voltage"]}'.strip())
    ...
    # ---- Fill C7.2 (proposed loads) ----
    # Sample sheet: the first data row has label "Load No" above it
    start_row = _find_first_load_row(ws)
    for i, load in enumerate(data.get("proposed_loads", [])):
        r = start_row + i
        ws.cell(row=r, column=1,  value=load.get("load_no", i + 1))
        ws.cell(row=r, column=3,  value=load.get("equipment", ""))
        ws.cell(row=r, column=12, value=load.get("power_w", 0))
        ws.cell(row=r, column=15, value=load.get("current_a", 0))
        ws.cell(row=r, column=21, value=load.get("cable_awg", ""))
        ws.cell(row=r, column=27, value=load.get("breaker_a", ""))
